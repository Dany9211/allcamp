...
# --- FUNZIONI COMUNI PER CALCOLO STATISTICHE ---

def calcola_winrate(df, col_risultato, home_as_home=True):
    """Calcola il WinRate per i mercati 1X2."""
    df_valid = df[df[col_risultato].notna() & (df[col_risultato].str.contains("-"))]
    risultati = {"Vittorie": 0, "Pareggi": 0, "Sconfitte": 0}
    for ris in df_valid[col_risultato]:
        try:
            home, away = map(int, ris.split("-"))
            if home_as_home: # Per la squadra di casa che gioca in casa
                if home > away:
                    risultati["Vittorie"] += 1
                elif home < away:
                    risultati["Sconfitte"] += 1
                else:
                    risultati["Pareggi"] += 1
            else: # Per la squadra in trasferta che gioca fuori casa
                if home < away:
                    risultati["Vittorie"] += 1
                elif home > away:
                    risultati["Sconfitte"] += 1
...
def calcola_btts(df_to_analyze):
    """Calcola le statistiche BTTS SI/NO FT."""
    ...
    btts_count = ((df_btts["gol_home_ft"] > 0) & (df_btts["gol_away_ft"] > 0)).sum()
    no_btts_count = len(df_btts) - btts_count
    total_matches = len(df_btts)
    data = [
        ["BTTS SI", btts_count, round((btts_count / total_matches) * 100, 2) if total_matches > 0 else 0],
        ["BTTS NO", no_btts_count, round((no_btts_count / total_matches) * 100, 2) if total_matches > 0 else 0]
    ]
...
def calcola_over_2_5(df_to_analyze):
    """Calcola le statistiche Over 2.5."""
    if df_to_analyze.empty:
        return pd.DataFrame()
    df_over = df_to_analyze.copy()
    df_over["gol_home_ft"] = pd.to_numeric(df_over["gol_home_ft"], errors='coerce')
    df_over["gol_away_ft"] = pd.to_numeric(df_over["gol_away_ft"], errors='coerce')
    df_over["tot_goals_ft"] = df_over["gol_home_ft"] + df_over["gol_away_ft"]
    over_count = (df_over["tot_goals_ft"] > 2.5).sum()
    under_count = len(df_over) - over_count
    total_matches = len(df_over)
    data = [
        ["Over 2.5", over_count, round((over_count / total_matches) * 100, 2) if total_matches > 0 else 0],
        ["Under 2.5", under_count, round((under_count / total_matches) * 100, 2) if total_matches > 0 else 0]
    ]
    df_stats = pd.DataFrame(data, columns=["Mercato", "Conteggio", "Percentuale %"])
    df_stats["Odd Minima"] = df_stats["Percentuale %"].apply(lambda x: round(100/x, 2) if x > 0 else "-")
    return df_stats
...
# --- FUNZIONE NUOVA PAGINA: ANALISI SQUADRE ---
def team_analysis_page(df):
    st.title("Analisi Squadre (Home e Away)")
    st.write("Seleziona una squadra di casa e una in trasferta per analizzare le loro prestazioni in casa e fuori casa.")

    all_teams = sorted(list(set(df['home_team'].dropna().unique()) | set(df['away_team'].dropna().unique())))
    
    col1, col2 = st.columns(2)
    with col1:
        home_team = st.selectbox("Seleziona Squadra di Casa", ["Seleziona..."] + all_teams)
    with col2:
        away_team = st.selectbox("Seleziona Squadra in Trasferta", ["Seleziona..."] + all_teams)

    if home_team != "Seleziona..." and away_team != "Seleziona...":
        st.markdown("---")
        
        # Analisi della squadra di casa quando gioca in casa
        st.subheader(f"Statistiche di {home_team} quando gioca in casa")
        home_matches_df = df[df["home_team"] == home_team].copy()
        
        if not home_matches_df.empty:
            col_met1, col_met2, col_met3 = st.columns(3)
            col_met1.metric("Partite Giocate", len(home_matches_df))
            
            # Calcolo e visualizzazione media gol
            home_matches_df["gol_home_ft"] = pd.to_numeric(home_matches_df["gol_home_ft"], errors='coerce')
            home_matches_df["gol_away_ft"] = pd.to_numeric(home_matches_df["gol_away_ft"], errors='coerce')
            avg_gol_fatti = home_matches_df["gol_home_ft"].mean()
            avg_gol_subiti = home_matches_df["gol_away_ft"].mean()
            col_met2.metric("Gol Fatti (media)", f"{avg_gol_fatti:.2f}")
            col_met3.metric("Gol Subiti (media)", f"{avg_gol_subiti:.2f}")

            # Calcolo e visualizzazione statistiche richieste
            st.write("### Risultato FT (1X2)")
            styled_winrate = calcola_winrate(home_matches_df, "risultato_ft", home_as_home=True).style.background_gradient(cmap='RdYlGn', subset=['WinRate %'])
            st.dataframe(styled_winrate)

            col_stats1, col_stats2 = st.columns(2)
            with col_stats1:
                st.write("### BTTS FT")
                styled_btts = calcola_btts(home_matches_df).style.background_gradient(cmap='RdYlGn', subset=['Percentuale %'])
                st.dataframe(styled_btts)
            with col_stats2:
                st.write("### Over 2.5 FT")
                styled_over = calcola_over_2_5(home_matches_df).style.background_gradient(cmap='RdYlGn', subset=['Percentuale %'])
                st.dataframe(styled_over)
        else:
            st.warning(f"Nessuna partita trovata per {home_team} in casa.")
        
        st.markdown("---")
        
        # Analisi della squadra in trasferta quando gioca fuori casa
        st.subheader(f"Statistiche di {away_team} quando gioca fuori casa")
        away_matches_df = df[df["away_team"] == away_team].copy()

        if not away_matches_df.empty:
            col_met1, col_met2, col_met3 = st.columns(3)
            col_met1.metric("Partite Giocate", len(away_matches_df))
            
            # Calcolo e visualizzazione media gol
            away_matches_df["gol_home_ft"] = pd.to_numeric(away_matches_df["gol_home_ft"], errors='coerce')
            away_matches_df["gol_away_ft"] = pd.to_numeric(away_matches_df["gol_away_ft"], errors='coerce')
            avg_gol_fatti = away_matches_df["gol_away_ft"].mean()
            avg_gol_subiti = away_matches_df["gol_home_ft"].mean()
            col_met2.metric("Gol Fatti (media)", f"{avg_gol_fatti:.2f}")
            col_met3.metric("Gol Subiti (media)", f"{avg_gol_subiti:.2f}")

            # Calcolo e visualizzazione statistiche richieste
            st.write("### Risultato FT (1X2)")
            styled_winrate = calcola_winrate(away_matches_df, "risultato_ft", home_as_home=False).style.background_gradient(cmap='RdYlGn', subset=['WinRate %'])
            st.dataframe(styled_winrate)

            col_stats1, col_stats2 = st.columns(2)
            with col_stats1:
                st.write("### BTTS FT")
                styled_btts = calcola_btts(away_matches_df).style.background_gradient(cmap='RdYlGn', subset=['Percentuale %'])
                st.dataframe(styled_btts)
            with col_stats2:
                st.write("### Over 2.5 FT")
                styled_over = calcola_over_2_5(away_matches_df).style.background_gradient(cmap='RdYlGn', subset=['Percentuale %'])
                st.dataframe(styled_over)
        else:
            st.warning(f"Nessuna partita trovata per {away_team} fuori casa.")

# --- INIZIO SCRIPT E SELETTORE PAGINA ---

# Ho spostato le funzioni di calcolo in alto, ma per non sovrapporle a quelle
# della pagina principale ho creato delle versioni più semplici per l'analisi
# delle singole squadre. Questo evita di copiare e incollare molto codice.

# Selezionatore di pagina nella sidebar
st.sidebar.title("Navigazione")
page = st.sidebar.selectbox("Seleziona una pagina", ["Analisi Completa", "Analisi Squadre"])

# Logica per mostrare la pagina corretta
if page == "Analisi Completa":
    # Questo è il tuo codice originale, incapsulato in una funzione fittizia
    # per la chiarezza della struttura multi-pagina.
    st.title("Analisi Tabella allcamp")
    st.write(f"**Righe iniziali nel dataset:** {len(df)}")
    
    # Questo è il blocco principale del tuo codice precedente
    filters = {}
    ...
    def mostra_distribuzione_timeband(df_to_analyze):
        ...
        for (start, end), label in zip(intervalli, label_intervalli):
            ...
            perc_1_plus = round((partite_con_gol / totale_partite) * 100, 2) if totale_partite > 0 else 0
            odd_min_1_plus = round(100 / perc_1_plus, 2) if perc_1_plus > 0 else "-"
